...
export default {
  ...
  description: 'Reject a group join request (admin/owner only)',
  ...
  async execute({ sock, chatId, senderId, lid, phoneNumber, args, isGroup, lang }) {
    ...
    try {
      ...
      await sock.sendMessage(chatId, {
        text: formatMessage(lang === 'de' ? 
          `✅ Anfrage von ${request.phoneNumber} wurde abgelehnt!` :
          `✅ Request from ${request.phoneNumber} has been rejected!`)
      });
    } catch (error) {
      console.error('Error rejecting join request:', error);
      await sock.sendMessage(chatId, {
        text: formatMessage(lang === 'de' ? 
          `❌ Fehler beim Ablehnen der Anfrage! Möglicherweise wurde sie bereits bearbeitet.` :
          `❌ Error rejecting request! It may have already been processed.`)
      });
    }
  }
};
